...
import {
  ...
  buildAgentTaskConversationHistorySection,
...
const TASK_SCHEMA_BLOCK = `{
  ...
  "output_description": string, // 2–3 sentences: what you will put in the deliverable document and what purpose it will serve for the project — this is what makes each task's deliverable distinct
...
export function buildAgentTasksPrompt({
  ...
  sections.push(
    ...
    "- Every task produces a tangible deliverable document the user can review. Describe the artifact in `output_description` — spell out exactly what sections or content you would write, and what purpose that deliverable serves for the project (e.g. \"A document listing the three highest-risk assumptions in the current requirements, so the user can validate them before building\"). Do not repeat the task title — describe the artifact itself.",
