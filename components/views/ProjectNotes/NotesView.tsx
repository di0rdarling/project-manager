...
export default function NotesView({
  projectId,
  featureId,
}: Readonly<NotesViewProps>) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const notesPath = featureId
  ...
  useEffect(() => {
    if (searchParams.get("folder")) {
      router.replace(notesPath);
    }
  }, [notesPath, router, searchParams]);
  ...
  const canFetch =
    !isProjectPending &&
    !isProjectError &&
    (!featureId || (!isFeaturePending && !isFeatureError));
  ...
  const isPending =
    isProjectPending ||
    (Boolean(featureId) && isFeaturePending) ||
    isNotesPending ||
    isFoldersPending;
  const isError =
    isProjectError ||
    (Boolean(featureId) && isFeatureError) ||
    isNotesError ||
    isFoldersError;
  const error = projectError ?? featureError ?? notesError ?? foldersError;
